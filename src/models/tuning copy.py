...
def objective(trial):
    ...
    epochs     = trial.suggest_int("epochs", 1, 2, 4)
...
def run_hyperparameter_search():
    """Launch Optuna optimization with MLflow logging and produce full summary table."""
    ...
    logger.info("Accuracy: %.4f", study.best_value)

    # ------------------------------------------------------
    # SUMMARY TABLE FOR ALL TRIALS
    # ------------------------------------------------------
    rows = []
    ...
    # Pretty table for log file
    logger.info("\n=========== ALL TRIAL RESULTS ===========\n%s",
                df.to_string(index=False))

    # CSV export
    df.to_csv("optuna_trial_results.csv", index=False)
    logger.info("Saved Optuna full results to optuna_trial_results.csv")
